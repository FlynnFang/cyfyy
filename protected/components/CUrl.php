...
class CUrl extends CApplicationComponent {
        ...
        if (! empty ( $validOptions )) {
            foreach ( $value as $key => $val ) {
                ...
                if ((! is_array ( $validOptions [$key] ['type'] ) && ($type != $validOptions [$key] ['type'])) || (is_array ( $validOptions [$key] ['type'] ) && ! in_array ( $type, $validOptions [$key] ['type'] ))) {
                    throw new CException ( Yii::t ( 'Curl', '{k} must be of type {t}', array ('{k}' => $key, '{t}' => $validOptions [$key] ['type'] ) ) );
                }
    ...
    /*
        ...
        */
    public function run($url, $GET = TRUE, $POSTSTRING = array(), $clean = true) {
        $this->setUrl ( $url );
        if (! $this->url)
        ...
        if ($GET == TRUE) {
            ...
            $this->setOption ( CURLOPT_POSTFIELDS, $clean ? $this->cleanPost ( $POSTSTRING ) : $POSTSTRING);
        }
    ...
    protected function cleanPost($string) {
		$thePostString = '';
        ...
        if (is_array ( $string )) {
            ...
            $thePostString = ltrim($thePostString, '&');
        } else {
            $thePostString = $string ;
        }
          
        return urldecode($thePostString);
    }
